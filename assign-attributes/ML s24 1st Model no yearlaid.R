...
# Train the model
rf.fit41 <- train(DIAMETE ~ n2_D + n2_M + n2_Y + PURPOSE + DAS + PROPERTY_TYPE,
                  tuneLength = 1,
...
# variable importance
rf.fit2 <- cforest(DIAMETE ~ n2_D + n2_M + n2_Y + PURPOSE + DAS + PROPERTY_TYPE, 
...
## select notional pdas with same DAS in model selection -------------------------
...
s24_2nd_DAS <- s243_filtered2 %>% anti_join(s24_1st_DAS, by = "DAS")

## predict diameter
tic()
# model 1
rf.prednotional_s24diam <- predict(rf.fit41_DAS, newdata = s24_1st_DAS)
toc()
