...
rf.fit41 %>% 
  write_rds("G:/Desktop/STW PDaS/Git/pdas_mapping/assign-attributes/S24diam_model County noY.RDS")

# Load model --------------------------------------------------------------

rf.fit41_3rdCounty <- read_rds("G:/Desktop/STW PDaS/Git/pdas_mapping/assign-attributes/S24diam_model County noY.RDS")

rf.preds24 <- predict(rf.fit41_3rdCounty, newdata = testdata.base2)
...
s24_3rd_County <- s243 %>%
  ...
  anti_join(s24_2nd_DAS, by = "index") %>% 
  as_tibble() %>% 
...
# band the n2_D
s24_3rd_County <- rollmeTibble(df1 = s24_3rd_County,
                                values = diameterclasses,
                                col1 = "n2_D",
                                method = 'nearest') %>%
  select(-n2_D, -band, -merge) %>% 
...
s24_3rd_County$n2_M[s24_3rd_County$n2_M == ""] <- NA
unique(s24_3rd_County$n2_M)


## remove the NA
s24_3rd_County <- s24_3rd_County %>% 
  na.omit()
summary(s24_3rd_County)

## predict diameter
tic()
rf.prednotional_s24diam3rd <- predict(rf.fit41_3rdCounty, newdata = s24_3rd_County)
toc()


# not predicted at all
not_predicted_s24 <- s243 %>% anti_join(s24_1st_DAS, by = "index") %>% 
  anti_join(s24_2nd_DAS, by = "index") %>% 
  anti_join(s24_3rd_County, by = "index") %>% 
  as_tibble() %>% 
  dplyr::filter(ConNghY <= 1937)
