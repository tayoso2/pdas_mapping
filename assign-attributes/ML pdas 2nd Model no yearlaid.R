...
rf.fitPDaSdia_County_noY <- read_rds("G:/Desktop/STW PDaS/Git/pdas_mapping/assign-attributes/PDaSdiam_model County noY.RDS")

rf.pred1 <- predict(rf.fitPDaSdia_County_noY, newdata = testdata.base)
caret::confusionMatrix(rf.pred1, testdata.base$DIAMETE) 

# Mat
rf.fitMAT_County_noY <- read_rds("G:/Desktop/STW PDaS/Git/pdas_mapping/assign-attributes/PDaSmat_model County noY.RDS")
...
pdas_2nd_County_noY <- pdas4 %>% anti_join(pdas_1st_DAS, by = "index") %>% 
...
# band the n2_D
pdas_2nd_County_noY <- rollmeTibble(df1 = pdas_2nd_County_noY,
                                values = diameterclasses,
                                col1 = "n2_D",
                                method = 'nearest') %>%
  select(-n2_D, -band, -merge) %>% 
...
## band the n2_M
pdas_2nd_County_noY <- pdas_2nd_County_noY %>%
...
pdas_2nd_County_noY$n2_M[pdas_2nd_County_noY$n2_M == ""] <- NA
unique(pdas_2nd_County_noY$n2_M)


## remove the NA
pdas_2nd_County_noY <- pdas_2nd_County_noY %>% 
  dplyr::filter(!is.na(n2_M))
summary(pdas_2nd_County_noY)
#pdas4_filtered3 <- dplyr::filter(pdas4_filtered2, grepl(nonafactors.pdas, DAS))
...
tic()
# model 2
rf.prednotionalpdas_mat2nd_noY <- predict(rf.fitMAT_County_noY, newdata = pdas_2nd_County_noY)
# toc()

## predict diameter
tic()
# model 2
rf.prednotionalpdas_diam2nd_noY <- predict(rf.fitPDaSdia_County_noY, newdata = pdas_2nd_County_noY)
# toc()
