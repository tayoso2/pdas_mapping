...
props_with_county_and_postcode <- fread(paste0(link_e,"props_with_county_and_postcode.csv"))
...
toc()
s24_assetbase$pipe_uq_id %>% unique()
...
rm(s24_assetbase_fid)

# tic()
# listofgrids <- createGrid(mainshapes = s24_da0, dlx, XCount = 10, YCount = 10,
#                           buffer = 300, mainexpand = F, extrasexpand = T, removeNull = T, report = T)
# toc()
# 
# tic()
# s24_da_pc <- get_nearest_features_using_grid(listofgrids, columns = c("Postcod","Year"))
# toc()

# s24_da0 <- s24_da0[,c("PURPOSE", "propgrp",  "drwng__", "directn", "pipe_id", "new_length", "fid", "PROPERTY_TYPE", "res_buil_1", "pipe_uq_id", "OBJECTID", "ID_DA_CODE")]
s24_da0 <- s24_da0[,c("fid", "PROPERTY_TYPE", "res_buil_1", "pipe_uq_id", "OBJECTID", "ID_DA_CODE")]
s24_da_pc_1pt <- divide_and_conquer(s24_da0[1:2000000,],get_nearest_features,dlx,"pipe_uq_id","Postcod","Year",splits = 10) # add postcode and Year
s24_da_pc_2pt <- divide_and_conquer(s24_da0[2000001:3836354,],get_nearest_features,dlx,"pipe_uq_id","Postcod","Year", splits = 10) # add postcode and Year
s24_da_pc <- rbind(s24_da_pc_1pt,s24_da_pc_2pt)
...
which_is_nearest$pipe_uq_id %>% unique()
...
s24_da_pc <- s24_da_pc %>% as.data.frame() %>% select(-geometry) %>% 
  left_join(s24_assetbase[,c("PURPOSE", "propgrp",  "drwng__", "directn", "pipe_id", "new_length", "pipe_uq_id","geometry")], by = "pipe_uq_id") %>% 
  st_as_sf(crs = 27700)  
s24_da_pc_ma_2 <- s24_da_pc %>% left_join(my_ans[,c("pipe_uq_id", "id_1", "Tag", "n2_M", "n2_D", "n2_Y")], by = "pipe_uq_id")
s24_da_pc$pipe_uq_id %>% unique()
s24_da_pc_ma_2$pipe_uq_id %>% unique()
...
## save and load image
# save.image("D:/STW PDAS/Phase 2 datasets/s24_d_m_y.rdata")
# load("D:/STW PDAS/Phase 2 datasets/s24_d_m_y.rdata")
...
rf.pred1_s24_1 <- predict(rf.fits24dia_DAS, newdata = s24_da_pc_ma_7[1:100000,]) %>% as.data.frame()
rf.pred1_s24_2 <- predict(rf.fits24dia_DAS, newdata = s24_da_pc_ma_7[100001:200000,]) %>% as.data.frame()
rf.pred1_s24_3 <- predict(rf.fits24dia_DAS, newdata = s24_da_pc_ma_7[200001:300000,]) %>% as.data.frame()
rf.pred1_s24_4 <- predict(rf.fits24dia_DAS, newdata = s24_da_pc_ma_7[300001:400000,]) %>% as.data.frame()
rf.pred1_s24_5 <- predict(rf.fits24dia_DAS, newdata = s24_da_pc_ma_7[400001:500000,]) %>% as.data.frame()
rf.pred1_s24_6 <- predict(rf.fits24dia_DAS, newdata = s24_da_pc_ma_7[500001:551210,]) %>% as.data.frame()
rf.pred1_s24 <- rbind(rf.pred1_s24_1,rf.pred1_s24_2,rf.pred1_s24_3,rf.pred1_s24_4,rf.pred1_s24_5,rf.pred1_s24_6)
names(rf.pred1_s24) <- "DIAMETE"
rm(rf.pred1_s24_1)
rm(rf.pred1_s24_2)
rm(rf.pred1_s24_3)
rm(rf.pred1_s24_4)
...
results_s24 <- rbind(result_pred_1_s24,result_pred_2_s24,result_pred_3_s24) %>% distinct()
results_s24 <- results_s24[!duplicated(results_s24$geometry),]
summary(results_s24)

# rename building polygon age column and edit the yearlaid column to median 
results_s24 <- results_s24 %>%
  dplyr::rename(YEARLAI = res_buil_1) %>%
...
results_s24_withna <- results_s24 %>% filter(is.na(YEARLAI))
results_s24_withoutna <- results_s24 %>% filter(!is.na(YEARLAI))

# infill assets with NA in YEARLAI using st_nearest_feature
results_s24_withna_yearlaid <- results_s24_withoutna[st_nearest_feature(results_s24_withna,results_s24_withoutna), "YEARLAI"]
results_s24_withna$YEARLAI <- results_s24_withna_yearlaid$YEARLAI
...
results_s24_withoutna$index <- seq.int(nrow(results_s24_withoutna))
st_geometry(results_s24_withoutna) <- NULL
# results_s24_withna_yearlaid2 <- results_s24_withna_yearlaid %>% 
#   select(-YEARLAI) %>% 
#   left_join(results_s24_withoutna[,c("index","YEARLAI")], by = "index")
results_s24_withna <- results_s24_withna[,my_columns] %>% st_sf() %>% st_set_crs(27700)
results_s24 <- rbind(results_s24_withoutna,results_s24_withna)
results_s24 <- results_s24[!duplicated(results_s24$geometry),]

summary(results_s24)
st_write(results_s24,"D:/STW PDAS/s24_phase_2/s24_lines_with_attrs_25012021.shp")


# save.image("D:/STW PDAS/Phase 2 datasets/s24_d_m_y_25012021.rdata")
# load("D:/STW PDAS/Phase 2 datasets/s24_d_m_y_25012021.rdata")
