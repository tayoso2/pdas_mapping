...
noNAssmall <- noNAs %>%
  st_drop_geometry() %>% 
  select(PIPEID, PURPOSE, County, Year,
...
rf.fitPDaSdia_County <- train(DIAMETE ~ n2_D + n2_M + n2_Y + PURPOSE + County + PROPERTY_TYPE + Year,
                              tuneLength = 5,
                              data = traindata.base,
                              method = "ranger",
...
tic()
rf.predPDaSdia <- predict(rf.fitPDaSdia_County, newdata = testdata.base)
toc()
caret::confusionMatrix(rf.predPDaSdia, testdata.base$DIAMETE) # 68% with county and Property type and year with NIR of 57%


# variable importance
rf.fit1 <- cforest(DIAMETE ~ n2_D + n2_M + n2_Y + PURPOSE + County + PROPERTY_TYPE + Year, 
...
rf.fitMAT_County <- train(MATERIA ~ n2_D + n2_M + n2_Y + PURPOSE + County + PROPERTY_TYPE + Year,
                          tuneLength = 5,
...
tic()
rf.predMAT <- predict(rf.fitMAT_County, newdata = testdata.baseMAT)
toc()
caret::confusionMatrix(rf.predMAT, testdata.baseMAT$MATERIA) # county + ppttype + year (95.2% and NIR 89.9%)

# variable importance
rf.fit21 <- cforest(MATERIA ~ n2_D + n2_M + n2_Y + PURPOSE + County + PROPERTY_TYPE + Year, 
...
rf.fitPDaSdia_County <- read_rds("G:/Desktop/STW PDaS/Git/pdas_mapping/assign-attributes/PDaSdiam_model County.RDS")

rf.pred1 <- predict(rf.fitPDaSdia_County, newdata = testdata.base)
caret::confusionMatrix(rf.pred1, testdata.base$DIAMETE) 

# Mat
rf.fitMAT_County <- read_rds("G:/Desktop/STW PDaS/Git/pdas_mapping/assign-attributes/PDaSmat_model County.RDS")
...
pdas_2nd_County <- pdas4 %>% anti_join(pdas_1st_DAS, by = "index") %>% 
  as_tibble() %>% 
  ...
  dplyr::mutate(PROPERTY_TYPE = ifelse(PROPERTY_TYPE == "Detatched","Detached",PROPERTY_TYPE),
                ...
                County = as.factor(County),
                DAS = as.factor(DA)) %>% 
  dplyr::select(index, n2_D, n2_M, n2_Y, PURPOSE, County, PROPERTY_TYPE, Year) %>% 
  na.omit()


# band the n2_D
pdas_2nd_County <- rollmeTibble(df1 = pdas_2nd_County,
                                values = diameterclasses,
                                col1 = "n2_D",
                                method = 'nearest') %>%
  select(-n2_D, -band, -merge) %>% 
...
## band the n2_M
pdas_2nd_County <- pdas_2nd_County %>%
...
pdas_2nd_County$n2_M[pdas_2nd_County$n2_M == ""] <- NA
unique(pdas_2nd_County$n2_M)

## remove the NA
pdas_2nd_County <- pdas_2nd_County %>% 
  dplyr::filter(!is.na(n2_M))
summary(pdas_2nd_County)
...
tic()
# model 2
rf.prednotionalpdas_mat2nd <- predict(rf.fitMAT_County, newdata = pdas_2nd_County)
toc()

## predict diameter
tic()
# model 2
rf.prednotionalpdas_diam2nd <- predict(rf.fitPDaSdia_County, newdata = pdas_2nd_County)
toc()
