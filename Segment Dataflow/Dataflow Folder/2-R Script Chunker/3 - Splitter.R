...
if (dataflow == TRUE) {
  ...
  fold_in_pipes  <- file.path("D:/STW PDAS/Git/pdas_mapping/Segment Dataflow/Test_data")
  #fold_in_pipes  <- file.path("C://PDaS_Phase2//Dataflow Execution//Dataflow Execution Folder//2-ShapePipes")
  
  
  fold_in_source <- file.path("D:/STW PDAS/Git/pdas_mapping/Segment Dataflow/Dataflow Folder/3-R Script Source")
  ...
  fold_out_pipes <- file.path("D:/STW PDAS/Git/pdas_mapping/Segment Dataflow/fold_out")
...
atts <- my_pipe %>% dplyr::mutate(pipe_id = row_number()) %>% as.data.frame()
...
rbind(something2,new_pipe_3_0_upper_thres) %>%
  ...
  st_write(paste0(fold_out_pipes,"/merged_pipe_da.shp"))
