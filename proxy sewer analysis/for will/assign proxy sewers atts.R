...
proxy_sewer_with_age_dm_bd_bm$length_m <- st_length(proxy_sewer_with_age_dm_bd_bm$geometry)
proxy_sewer_with_age_dm_bm_bd_length <- rollmeTibble(df1 = proxy_sewer_with_age_dm_bd_bm,
                                           values = lengthclasses,
                                           col1 = "length_m",
                                           method = 'nearest') %>%
  dplyr::select(-band,-merge, - length_m) %>% 
...
proxy_sewer_with_age_dm_bm_bd_length <- proxy_sewer_with_age_dm_bm_bd_length %>% st_sf() %>% st_set_crs(27700)
proxy_sewer_with_b_d_pcd <- get_nearest_features(proxy_sewer_with_age_dm_bm_bd_length, the_postcode_2[, c("pcd_id", "nrst_p_")],
                                                 "id_0", "pcd_id", "nrst_p_") # the model was trained on this postcode data, 
...
mapped_ml_model <- go_into_model %>% 
  as_tibble() %>%
  mutate(res_buil_1 = modal_value) %>% 
  select(Bnd_D_2,Type,res_buil_1,Bnd_M_2,Bnd_Length, aprx_pstcd_2ch, aprx_pstcd_3ch, Cnty_st) %>%
  ...
  distinct() %>% 
  mutate(Cnty_st = as.factor(Cnty_st),
...
# load the models
mapped_ml_model_rffit_diam <- readRDS("D:/STW PDAS/Git/pdas_mapping/proxy sewer analysis/ps_attrs_models/mapped_ml_model_rffit_diam_2.rds") # 83.4, 72 nir
mapped_ml_model_rffit_mat <- readRDS("D:/STW PDAS/Git/pdas_mapping/proxy sewer analysis/ps_attrs_models/mapped_ml_model_rffit_mat_2.rds") # 72, 49 nir
